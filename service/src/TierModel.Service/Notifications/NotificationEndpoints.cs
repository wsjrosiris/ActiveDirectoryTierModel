...
public static partial class NotificationEndpoints
{
    ...
    private static string? ValidateTarget(ChannelType type, string target)
    {
        if (type == ChannelType.Email)
        {
            ...
            if (parts.Length == 0) return L.T("Mindestens eine E-Mail-Adresse angeben.");
            var bad = parts.FirstOrDefault(p => !MailboxAddress.TryParse(p, out var m) || !m.Address.Contains('@'));
            return bad is null ? null : L.F("'{0}' ist keine gültige E-Mail-Adresse.", bad);
        }
        ...
        return Uri.TryCreate(target, UriKind.Absolute, out var u) && (u.Scheme == "https" || (u.Scheme == "http" && u.IsLoopback))
            ? null : L.T("Bitte eine vollständige https-Adresse angeben.");
    }
    ...
    public static void MapNotificationEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        g.MapPost("/channels", async (ChannelRequest r, HttpContext ctx, AppDbContext db, SettingsService settings, ChangeLogService log, SiemForwarder forwarder) =>
        {
            ...
            log.Add(ctx.User.UserName(), "notification.create", "notification", channel.Id.ToString(), L.PF("Benachrichtigungskanal '{0}' ({1}) angelegt", channel.Name, channel.Type));
            await db.SaveChangesAsync();
            ...
            SiemForwarder forwarder) =>
        {
            ...
            Apply(channel, r.Events);
            log.Add(ctx.User.UserName(), "notification.update", "notification", id.ToString(), L.PF("Benachrichtigungskanal '{0}' geändert", channel.Name));
            await db.SaveChangesAsync();
            forwarder.Invalidate();
            return Results.Ok(ChannelDto.From(channel, settings.Secrets, forwarder));
        });
        ...
        g.MapDelete("/channels/{id:long}", async (long id, HttpContext ctx, AppDbContext db, ChangeLogService log, SiemForwarder forwarder) =>
        {
            ...
            log.Add(ctx.User.UserName(), "notification.delete", "notification", id.ToString(), L.PF("Benachrichtigungskanal '{0}' gelöscht", channel.Name));
            await db.SaveChangesAsync();
            forwarder.Invalidate();
            forwarder.Forget(id);
            ...
            NotificationService notifications, ChangeLogService log, CancellationToken ct) =>
        {
            ...
            try
            {
                ...
                channel.LastError = null;
                log.Add(ctx.User.UserName(), "notification.test", "notification", id.ToString(), L.PF("Testnachricht über '{0}' gesendet", channel.Name));
            ...
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
                ...
                await db.SaveChangesAsync(ct);
                return Results.Problem(title: L.T("Testnachricht konnte nicht zugestellt werden"), detail: ex.Message, statusCode: 502);
            }
        });
        ...
        g.MapPut("/smtp", async (SmtpRequest r, HttpContext ctx, SettingsService settings, ChangeLogService log, AppDbContext db) =>
        {
            ...
            if (r.Security is not ("None" or "StartTls" or "SslOnConnect")) errors["security"] = [L.T("None, StartTls oder SslOnConnect.")];
            if (!string.IsNullOrWhiteSpace(r.From) && !(MailboxAddress.TryParse(r.From, out var from) && from.Address.Contains('@')))
                errors["from"] = [L.T("Ungültige Absenderadresse.")];
            if (!string.IsNullOrWhiteSpace(r.Host) && string.IsNullOrWhiteSpace(r.From)) errors["from"] = [L.T("Absenderadresse angeben.")];
            if (errors.Count > 0) return Results.ValidationProblem(errors);
            ...
            log.Add(ctx.User.UserName(), "smtp.update", "settings", null, L.PF("SMTP-Einstellungen geändert: {0}:{1} ({2})", config.Host, config.Port, config.Security));
            await db.SaveChangesAsync();
    ...
    {
        var errors = new Dictionary<string, string[]>();
        if (string.IsNullOrWhiteSpace(r.Name) || r.Name.Length > 100) errors["name"] = [L.T("Name angeben (max. 100 Zeichen).")];
        if (!Enum.IsDefined(r.Type)) errors["type"] = [L.T("Unbekannter Typ.")];
        ...
        else if (requireTarget && string.IsNullOrWhiteSpace(r.Target)) errors["target"] = [r.Type == ChannelType.Email ? L.T("Empfänger angeben.") : L.T("Adresse (URL) angeben.")];
        return errors;
    }
    ...
    public static SiemChannelConfig? BuildSiem(ChannelRequest r, SiemChannelConfig? existing, Dictionary<string, string[]> errors)
    {
        ...
        if (r.Type == ChannelType.Syslog)
        {
            ...
            if (s is null) { errors["syslog"] = [L.T("Syslog-Server angeben.")]; return null; }
            s = s with { Host = (s.Host ?? "").Trim() };
            if (s.Host.Length == 0 || !HostPattern().IsMatch(s.Host)) errors["syslog.host"] = [L.T("Gültigen Hostnamen oder IP-Adresse angeben.")];
            if (s.Port is < 1 or > 65535) errors["syslog.port"] = ["Port 1–65535."];
            if (!Enum.IsDefined(s.Protocol)) errors["syslog.protocol"] = [L.T("UDP, TCP oder TLS.")];
            if (!Enum.IsDefined(s.Format)) errors["syslog.format"] = [L.T("CEF oder RFC 5424.")];
        ...
        if (!Guid.TryParse(l.TenantId, out _)) errors["logAnalytics.tenantId"] = [L.T("Mandanten-ID als GUID angeben.")];
        if (!Guid.TryParse(l.ClientId, out _)) errors["logAnalytics.clientId"] = [L.T("Anwendungs-ID (Client-ID) als GUID angeben.")];
        if (!(Uri.TryCreate(l.EndpointUrl, UriKind.Absolute, out var u) && (u.Scheme == "https" || (u.Scheme == "http" && u.IsLoopback))))
            errors["logAnalytics.endpointUrl"] = [L.T("Vollständige https-Adresse des Datensammlungsendpunkts angeben.")];
        if (!DcrPattern().IsMatch(l.DcrImmutableId)) errors["logAnalytics.dcrImmutableId"] = [L.T("Unveränderliche ID der Regel im Format dcr-… (32 Hexadezimalzeichen).")];
        if (!StreamPattern().IsMatch(l.StreamName)) errors["logAnalytics.streamName"] = [L.T("Streamname im Format Custom-Tabelle, z. B. Custom-TierModel_CL.")];
        if (string.IsNullOrEmpty(secret)) errors["logAnalytics.clientSecret"] = [L.T("Geheimen Clientschlüssel angeben.")];
