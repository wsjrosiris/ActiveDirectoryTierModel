...
public static class ConfigArchive
{
    ...
    public static ImportSource Read(Stream stream, string label)
    {
        ...
        {
            throw new ImportFormatException("Die Datei ist kein gültiges ZIP-Archiv.");
        }
        using (zip)
        {
            if (zip.Entries.Count > MaxEntries)
                throw new ImportFormatException($"Das Archiv enthält zu viele Dateien (höchstens {MaxEntries}).");
            ...
            foreach (var entry in zip.Entries)
            {
                ...
                if (entry.Length > MaxEntryBytes)
                    throw new ImportFormatException($"„{name}“ ist zu groß (höchstens {MaxEntryBytes >> 20} MB je Datei).");
                total += entry.Length;
                if (total > MaxTotalBytes)
                    throw new ImportFormatException($"Das Archiv ist entpackt zu groß (höchstens {MaxTotalBytes >> 20} MB).");
            ...
            if (manifest is null)
                notices.Add("versions.json fehlt – die Versionsnummern der Quelle sind unbekannt.");
            else
            {
                try
                {
                ...
                {
                    notices.Add("versions.json ist beschädigt und wurde ignoriert.");
                }
            }
            if (sections.Count == 0 && invalid.Count == 0)
                throw new ImportFormatException("Das Archiv enthält keine Konfigurationsdateien (erwartet: config/tiermodel-*.json aus einem Export).");
            if (unknown.Count > 0)
                notices.Add($"{unknown.Count} unbekannte Datei(en) ignoriert: {string.Join(", ", unknown.Take(10))}{(unknown.Count > 10 ? " …" : "")}");
    ...
    public static string NormalizeEntryName(string fullName)
    {
        ...
        if (name.Length == 0 || name.StartsWith('/') || (name.Length > 1 && name[1] == ':') || name.Contains('\0')
            ...
            throw new ImportFormatException($"Unzulässiger Pfad im Archiv: „{fullName}“.");
        return name;
    }
    ...
    private static string ReadText(ZipArchiveEntry entry)
    {
        ...
        while ((read = s.Read(buffer, 0, buffer.Length)) > 0)
        {
            ...
            if (limited.Length > MaxEntryBytes)
                throw new ImportFormatException($"„{entry.FullName}“ ist zu groß (höchstens {MaxEntryBytes >> 20} MB je Datei).");
        }
    ...
    public static string? ParseObject(string text)
    {
        try
        {
            return JsonNode.Parse(text) is JsonObject ? null : "Der Inhalt ist kein JSON-Objekt.";
        }
        catch (JsonException ex)
        {
            return $"Ungültiges JSON: {ex.Message}";
        }
    }
}
