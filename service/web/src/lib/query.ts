...
export function errorMessage(e: unknown): string {
  ...
  return t('lib.query.unknownError')
}
...
export const queryClient = new QueryClient({
  ...
  queryCache: new QueryCache({
    onError: (err, query) => {
      ...
      if (query.state.data !== undefined) toast.error(t('lib.query.refreshFailed'), { description: errorMessage(err) })
    },
  }),
  mutationCache: new MutationCache({
    onError: (err, _v, _c, mutation) => {
      ...
      toast.error(err instanceof ApiError ? err.title : t('lib.query.error'), {
