...
export function applyTimingText(s: MaintenanceStatus | undefined): string | null {
  if (!s) return null
  if (s.activeFreeze) return `Sperrzeit „${s.activeFreeze.reason}“ bis ${formatDateTime(s.activeFreeze.to)} – Anwenden ist gesperrt.`
  if (s.allowedNow) return null
  return s.nextStart
    ? `Außerhalb eines Wartungsfensters: Der Lauf wird geplant und startet ${formatDateTime(s.nextStart)}${s.nextWindow ? ` (Fenster „${s.nextWindow.name}“)` : ''}.`
    : 'Außerhalb eines Wartungsfensters: Der Lauf wird geplant und startet im nächsten Wartungsfenster.'
}
...
export function MaintenanceNotice({ className, compact }: { className?: string; compact?: boolean }) {
  ...
  const link = isAdmin && !compact && (
    <Link to="/admin/wartungsfenster" className="font-medium underline underline-offset-2">Wartungsfenster verwalten</Link>
  )
  if (s.activeFreeze)
    return (
      <div role="alert" className={cn('flex gap-2 rounded-lg border border-rose-500/30 bg-rose-500/5 p-3 text-xs text-rose-900 dark:text-rose-200', className)}>
        ...
        <span>
          <span className="font-medium">Sperrzeit „{s.activeFreeze.reason}“</span> bis {formatDateTime(s.activeFreeze.to)} ({formatRelative(s.activeFreeze.to)}).
          Anwenden ist in dieser Zeit nicht möglich; Planungsläufe und Audits schon. {link}
        </span>
      </div>
    )
  ...
  if (s.allowedNow && s.currentWindow)
    return (
      <div className={cn('flex gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-3 text-xs text-emerald-900 dark:text-emerald-200', className)}>
        ...
        <span>
          <span className="font-medium">Wartungsfenster „{s.currentWindow.name}“ ist geöffnet</span> bis {formatDateTime(s.currentWindow.end)}. Der Lauf startet sofort. {link}
        </span>
      </div>
    )
  return (
    <div className={cn('flex gap-2 rounded-lg border border-sky-500/30 bg-sky-500/5 p-3 text-xs text-sky-900 dark:text-sky-200', className)}>
      ...
      <span>
        <span className="font-medium">Außerhalb eines Wartungsfensters.</span>{' '}
        {s.nextStart ? (
          <>Der Lauf wird geplant und startet automatisch <strong>{formatDateTime(s.nextStart)}</strong>{s.nextWindow && <> im Fenster „{s.nextWindow.name}“</>}.</>
        ) : (
          'Derzeit ist kein Wartungsfenster in Sicht.'
        )}{' '}
        {link}
      </span>
    </div>
