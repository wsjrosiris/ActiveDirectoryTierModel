...
export function useApplyPlan() {
  ...
  const deploy = useMutation({
    mutationFn: ({ req, planRunId }: { req: RunRequest; planRunId: number | null }) =>
      api.runs.deploy({ ...req, preferredDc: req.preferredDc.trim(), confirmApply: true, planRunId }),
    meta: { silent: true },
    onError: (e, { planRunId }) => {
  ...
  const start = async (req: RunRequest, planRunId: number | null, changes?: number) => {
    ...
    if (ok) deploy.mutate({ req, planRunId })
  }

  return { start, isPending: deploy.isPending, needsApproval: !!settings.data?.requireApproval }
}
