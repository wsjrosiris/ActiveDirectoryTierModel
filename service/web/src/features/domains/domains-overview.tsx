    ...
    <Card className="mb-4 overflow-hidden" data-testid="domains-overview">
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <div>
            <CardTitle>Alle Domänen</CardTitle>
            <CardDescription>Compliance und letzte Läufe je Domäne – zum Wechseln eine Domäne anklicken.</CardDescription>
          </div>
        </div>
      </CardHeader>
...
function DomainTile({ d, active, onSelect }: { d: DomainOverview; active: boolean; onSelect: () => void }) {
  const tiers = [0, 1, 2].map((t) => d.compliance?.find((c) => c.tier === t)?.score)
  return (
    <button
      type="button"
      onClick={onSelect}
    ...
    >
      <div className="flex min-w-0 items-start justify-between gap-2">
        ...
        {active ? (
          <span className="shrink-0 rounded-full bg-primary/10 px-2 py-0.5 text-[11px] font-medium text-primary">Aktuell</span>
        ) : (
      ...
      <div className="grid grid-cols-3 gap-2">
        {tiers.map((score, t) => (
          <div key={t} className="rounded-md bg-muted/50 px-2 py-1.5">
            <p className="text-[10.5px] text-muted-foreground">Tier {t}</p>
      ...
      <div className="grid gap-1 text-xs text-muted-foreground">
        <span className="flex min-w-0 items-center gap-1.5">
          ...
          <span className="truncate">
            {d.lastAudit
              ? `Audit ${formatRelative(d.lastAudit.at)}${d.lastAudit.driftCount ? ` · ${d.lastAudit.driftCount} Abweichung(en)` : d.lastAudit.status === 'Succeeded' ? ' · ohne Abweichung' : ' · fehlgeschlagen'}`
              : 'Noch kein Audit'}
          </span>
        </span>
        <span className="flex min-w-0 items-center gap-1.5">
          <ShieldUser className="size-3.5 shrink-0" />
          <span className="truncate">{d.lastMonitor ? `Überwachung ${formatRelative(d.lastMonitor.at)}` : 'Noch keine Überwachung'}</span>
        </span>
      </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {d.pendingApprovals > 0 && (
            <span className="inline-flex items-center gap-1 rounded-full bg-amber-500/15 px-2 py-0.5 text-[11px] font-medium text-amber-800 dark:text-amber-300">
              <Hourglass className="size-3" /> {d.pendingApprovals} Freigabe{d.pendingApprovals === 1 ? '' : 'n'}
            </span>
          )}
          {d.active > 0 && (
            <span className="inline-flex items-center gap-1 rounded-full bg-sky-500/15 px-2 py-0.5 text-[11px] font-medium text-sky-700 dark:text-sky-300">
              <Loader2 className="size-3 animate-spin" /> {d.active} aktiv
            </span>
          )}
          {d.setupNeeded && (
            <span className="inline-flex items-center gap-1 rounded-full bg-violet-500/15 px-2 py-0.5 text-[11px] font-medium text-violet-700 dark:text-violet-300">
              <Sparkles className="size-3" /> Beispielkonfiguration
            </span>
          )}
          {d.lastAudit?.status === 'Failed' && (
            <span className="inline-flex items-center gap-1 rounded-full bg-rose-500/15 px-2 py-0.5 text-[11px] font-medium text-rose-700 dark:text-rose-300">
              <AlertTriangle className="size-3" /> Audit fehlgeschlagen
            </span>
          )}
        </div>
      )}
