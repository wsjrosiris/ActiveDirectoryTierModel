...
> **Update (this fork, Unreleased):** Domains with localized group names (e.g. German `Domänen-Admins`)
> are **supported**. The configuration keeps the English names; built-in users and groups are resolved via their
> fixed SIDs/RIDs (`Get-TierModelWellKnownPrincipal`): domain-relative RIDs such as 512 *Domain Admins* or
> 516 *Domain Controllers*, BUILTIN SIDs `S-1-5-32-5xx` and NT AUTHORITY SIDs. Schema Admins, Enterprise Admins,
> Enterprise Key Admins and Enterprise Read-only Domain Controllers are built from the **forest root** domain SID,
> so child domains work as well. The Domain Admins prerequisite check, Deny-Apply entries of GPOs, user rights
> (`resolvableGroups`, `forestRootOnly`) and Windows LAPS groups use this resolution. The detected AD language is
> reported for information only. Groups without a fixed RID (e.g. *DnsAdmins*) are still looked up by name.
> The host operating system check accepts 19 languages. The rest of this page describes the original English-only
> behaviour and is kept for background.
